...
app.get('/api/assessments/user/:userId', async (req, res) => {
  try {
    const assessments = await Assessment.find({ userId: req.params.userId }).sort({
      completedAt: -1,
    });
...
describe('Assessment Integration Tests', () => {
  ...
  describe('GET /api/assessments/:id', () => {
    ...
    it('should get assessment by id', async () => {
      const response = await request(app).get(`/api/assessments/${testAssessmentId}`);
    ...
    it('should return 404 for non-existent assessment', async () => {
      ...
      const response = await request(app).get(`/api/assessments/${fakeId}`);
    ...
    it('should return 500 for invalid id format', async () => {
      const response = await request(app).get('/api/assessments/invalid-id');
  ...
  describe('GET /api/assessments/user/:userId', () => {
    ...
    it('should get all assessments for a user', async () => {
      const response = await request(app).get('/api/assessments/user/multi-assessment-user');
    ...
    it('should return empty array for user with no assessments', async () => {
      const response = await request(app).get('/api/assessments/user/new-user');
