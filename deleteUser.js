...
const deleteUserCompletely = async () => {
    ...
    try {
        ...
        const deletions = await Promise.all([
            ...
            MentorProfile.deleteMany({ userId }).then(r => ({ collection: 'MentorProfile', deleted: r.deletedCount })),

            // Delete user's bookings (as mentor or as user)
            MentorBooking.deleteMany({ $or: [{ userId }, { mentorId: userId }] }).then(r => ({ collection: 'MentorBooking', deleted: r.deletedCount })),

            // Delete user's mentor applications
            Application.deleteMany({ email: EMAIL_TO_DELETE.toLowerCase() }).then(r => ({ collection: 'Application', deleted: r.deletedCount })),

            // Delete user's assessments
            Assessment.deleteMany({ userId }).then(r => ({ collection: 'Assessment', deleted: r.deletedCount })),

            // Delete user's profile
            Profile.deleteMany({ userId }).then(r => ({ collection: 'Profile', deleted: r.deletedCount })),

            // Delete user's posts
            Post.deleteMany({ userId }).then(r => ({ collection: 'Post', deleted: r.deletedCount })),

            // Delete OTPs
            OTP.deleteMany({ email: EMAIL_TO_DELETE.toLowerCase() }).then(r => ({ collection: 'OTP', deleted: r.deletedCount })),

            // Delete email verifications
            EmailVerification.deleteMany({ email: EMAIL_TO_DELETE.toLowerCase() }).then(r => ({ collection: 'EmailVerification', deleted: r.deletedCount })),

            // Delete login verifications
            LoginVerification.deleteMany({ email: EMAIL_TO_DELETE.toLowerCase() }).then(r => ({ collection: 'LoginVerification', deleted: r.deletedCount })),

            // Delete user activity
            UserActivity.deleteMany({ userId }).then(r => ({ collection: 'UserActivity', deleted: r.deletedCount })),
        ]);
